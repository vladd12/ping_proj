	...
	case FUNC_SUCCESS:
		...
		case FUNC_SUCCESS:
			...
			case FUNC_SUCCESS:
				...
				case FUNC_SUCCESS:
					while (seq < steps) {
						switch (SendRequest(remote, listen, seq, start, IP_Str)) {
						case FUNC_SUCCESS:
							switch (GetReply(remote, local, listen, start, end, IP_Str)) {
							case FUNC_SUCCESS:
								break;
							case FUNC_ERROR:
					...
					/// TODO: Сделать подведение статистики как в обычном пинге
					break;
				case FUNC_ERROR:
					CloseProgram(TRUE, &listen);
					break;
...
int SendRequest(SOCKADDR_IN& remote, SOCKET& listen, WORD SeqNum, clock_t& start, string& ip_str) {
	try {
...
int GetReply(SOCKADDR_IN& remote, SOCKADDR_IN& local, SOCKET& listen, clock_t& start, clock_t& end, string& ip_str) {
	try {
